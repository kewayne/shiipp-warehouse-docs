---
title: "Create, Update, and Delete Customer Records"
sidebarTitle: "Manage Customers"
description: "GET, POST, PUT, DELETE /api/ManageCustomers.php — full CRUD for customer profiles with role-scoped access, pagination, and branch filtering."
---

The **ManageCustomers** endpoint provides full create, read, update, and delete (CRUD) access to customer profiles in your Shiipp account. All operations are scoped to the authenticated user's role — courier users can only access and modify customers within their own organization.
...
## GET — List and Search Customers

Retrieve a paginated, sortable list of customers. Supports filtering by branch and optional full-text search.
...
<ParamField query="search" type="string">
  Filter customers by user code, name, or email using a partial match.
</ParamField>

<ParamField query="branch" type="string">
  Filter results to customers assigned to a specific branch location.
</ParamField>
...
Append `?action=branches` to retrieve a distinct list of branch names available for the authenticated courier. This is useful for populating filter dropdowns in your UI.
...
```json
{
  ...
  "data": {
    "courier_customer_id": "cust-new-uuid"
  }
}
```

<Warning>
  If a customer with the same `user_code` already exists in the courier account, the API returns `409 Conflict`. Use the [PUT method](#put--update-a-customer) to update an existing record, or the [Upload Customers](/api-reference/customers/upload) endpoint for upsert behavior in bulk.
</Warning>
...
<Tabs>
  <Tab title="Query Parameter">
    Pass the customer ID as a query string parameter:
    ```
    DELETE /api/ManageCustomers.php?id=cust-uuid
    ```
  </Tab>
  <Tab title="Request Body">
    Pass the customer ID in the request body:
    ```json
    {
      "courier_customer_id": "cust-uuid"
    }
...
<Note>
  **Courier role users** can only list, create, update, and delete customers within their own courier organization. Admin and manager roles have cross-courier access and must pass `courier_id` when creating customers.
</Note>

| Operation | Courier Role | Manager Role | Admin Role |
|---|---|---|---|
| List customers | Own account only | All couriers | All couriers |
