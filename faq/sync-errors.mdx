---
title: "Troubleshoot Customer Sync and Webhook Errors"
...
<Accordion title="Sync failed with HTTP 500 or non-200">
  Shiipp logs the HTTP status code and the first 300 characters of your endpoint's response body for every sync attempt. To troubleshoot:
...
<Accordion title="Customers are missing after sync">
  ...
  - Confirm the `last_sync` timestamp updated — this is visible in **Settings** and indicates whether the sync actually ran.
</Accordion>
...
<Accordion title="My webhook endpoint isn't receiving calls">
  ...
  - Verify your webhook URL is saved in **Courier Settings** — use `api_url_add_package` for new package events and `api_url_status_update` for manifest status changes.
