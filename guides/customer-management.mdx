...
For approximate name matches — useful when you're unsure of the exact spelling — append `?advanced=true` to the Customers page URL. With advanced search enabled, Shiipp uses **Levenshtein distance** and **Soundex phonetic matching** to surface close matches even when names are slightly misspelled.
